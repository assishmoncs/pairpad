// Execute code controller — isolated worker / Judge0
...
const executeCode = async (req, res) => {
  try {
    ...
    const result = await executionService.executeCode(source_code, language, stdin || '');
    ...
    if (error.message.includes('API key') || error.message.includes('execution worker')) {
      return sendError(res, 503, 'Code execution service is unavailable or not configured.');
    }
