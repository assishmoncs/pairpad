...
const normalizeTests = (tests = [], hidden = false) => !Array.isArray(tests) ? [] : tests.slice(0, MAX_TESTS).map((test, index) => ({ id: typeof test?.id === 'string' && test.id ? test.id : crypto.randomUUID(), name: cleanText(test?.name, 100) || `Test ${index + 1}`, stdin: cleanText(test?.stdin, MAX_INPUT), expectedOutput: cleanText(test?.expectedOutput, MAX_EXPECTED), hidden }));
...
const validateConfig = (payload = {}) => {
  const title = cleanText(payload.title, MAX_TITLE); const description = cleanText(payload.description, MAX_DESCRIPTION); const durationMinutes = Number(payload.durationMinutes); const language = cleanText(payload.language, 30).toLowerCase(); const candidateId = typeof payload.candidateId === 'string' && payload.candidateId ? payload.candidateId : null;
  ...
  return { title, description, durationMinutes, language, candidateId, publicTests: normalizeTests(payload.publicTests, false), hiddenTests: normalizeTests(payload.hiddenTests, true) };
};

const requireOwner = (room, userId) => { if (getRoomRole(room, userId) !== ROLES.OWNER) throw Object.assign(new Error('Only the room owner can manage interview mode.'), { code: 'FORBIDDEN' }); };
...
const createOrUpdateInterview = async (room, userId, payload) => {
  ...
  room.interview = { ...config, status: 'draft', startedAt: null, pausedAt: null, endedAt: null, remainingSeconds: config.durationMinutes * 60 };
...
const startInterview = async (room, userId) => {
  ...
  room.interview.status = 'running'; room.interview.startedAt = new Date(); room.interview.pausedAt = null; room.interview.endedAt = null; room.interview.remainingSeconds = Math.max(1, room.interview.durationMinutes * 60); await room.save(); return room.interview;
};
const pauseInterview = async (room, userId) => {
  ...
  const elapsed = Math.floor((Date.now() - new Date(room.interview.startedAt).getTime()) / 1000); room.interview.remainingSeconds = Math.max(0, room.interview.remainingSeconds - elapsed); room.interview.pausedAt = new Date(); room.interview.status = room.interview.remainingSeconds === 0 ? 'ended' : 'paused'; if (room.interview.status === 'ended') room.interview.endedAt = new Date(); await room.save(); return room.interview;
};
const resumeInterview = async (room, userId) => { requireOwner(room, userId); if (room.interview?.status !== 'paused') throw Object.assign(new Error('Only a paused interview can be resumed.'), { code: 'INVALID_STATE' }); room.interview.status = 'running'; room.interview.startedAt = new Date(); room.interview.pausedAt = null; await room.save(); return room.interview; };
const endInterview = async (room, userId) => { requireOwner(room, userId); if (!['running', 'paused'].includes(room.interview?.status)) throw Object.assign(new Error('Only an active interview can be ended.'), { code: 'INVALID_STATE' }); room.interview.status = 'ended'; room.interview.endedAt = new Date(); await room.save(); return room.interview; };

const runTests = async (sourceCode, language, tests, includeExpected = false) => {
  const results = [];
  for (const test of tests) {
    const result = await executionService.executeCode(sourceCode, language, test.stdin || ''); const actual = (result.stdout || '').trim(); const expected = (test.expectedOutput || '').trim();
...
const submitCandidate = async (room, userId, sourceCode, language) => {
  ...
  if (!Number.isNaN(startedAt) && Date.now() - startedAt >= room.interview.remainingSeconds * 1000) { room.interview.status = 'ended'; room.interview.endedAt = new Date(); room.interview.remainingSeconds = 0; await room.save(); throw Object.assign(new Error('Interview time has expired.'), { code: 'INVALID_STATE' }); }
  ...
  const publicResults = await runTests(sourceCode, language, room.interview.publicTests, true); const hiddenResults = await runTests(sourceCode, language, room.interview.hiddenTests, false);
