...
const issueSession = async ({ userId, familyId, req }) => {
  ...
  await RefreshSession.create({
    ...
    familyId: familyId || decoded.familyId || crypto.randomUUID(),
    expiresAt: new Date(decoded.exp * 1000),
    userAgent: req?.get?.('user-agent')?.slice(0, 500),
    ipAddress: req?.ip?.slice(0, 100),
  });

  return { refreshToken, familyId: familyId || decoded.familyId };
};
...
const rotate = async ({ refreshToken, req }) => {
  ...
  if (!session) {
    const reuseFamily = decoded.familyId;
    if (reuseFamily) await RefreshSession.updateMany({ familyId: reuseFamily }, { $set: { revokedAt: new Date() } });
