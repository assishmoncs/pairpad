...
const formatTime = (seconds) => `${Math.floor(seconds / 60).toString().padStart(2, '0')}:${Math.max(0, seconds % 60).toString().padStart(2, '0')}`;
...
const InterviewPanel = ({ roomCode, room, currentRole, code, language, onRoomUpdated }) => {
  ...
  const [form, setForm] = useState({ title: '', description: '', durationMinutes: 60, language: language || 'javascript', candidateId: '', publicTests: [emptyTest()], hiddenTests: [emptyTest()] });
  ...
  const loadInterview = useCallback(async () => {
    try {
      ...
      if (next && isOwner) {
        setForm((current) => ({
          ...current,
          title: next.title || '', description: next.description || '', durationMinutes: next.durationMinutes || 60,
          language: next.language || language || 'javascript', candidateId: next.candidateId || '',
          publicTests: next.publicTests?.length ? next.publicTests.map(({ name, stdin, expectedOutput }) => ({ name, stdin, expectedOutput })) : [emptyTest()],
          hiddenTests: next.hiddenTests?.length ? next.hiddenTests.map(({ name, stdin, expectedOutput }) => ({ name, stdin, expectedOutput })) : [emptyTest()],
        }));
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    return () => { unsubscribe(); unsubscribeConfig(); };
  }, []);
  ...
  useEffect(() => {
    ...
    const calculate = () => {
      if (interview.status !== 'running' || !interview.startedAt) return Math.max(0, interview.remainingSeconds || 0);
  ...
  const updateTest = (kind, index, field, value) => setForm((current) => ({ ...current, [kind]: current[kind].map((test, i) => i === index ? { ...test, [field]: value } : test) }));
  const addTest = (kind) => setForm((current) => ({ ...current, [kind]: current[kind].length >= 50 ? current[kind] : [...current[kind], emptyTest()] }));
  const removeTest = (kind, index) => setForm((current) => ({ ...current, [kind]: current[kind].filter((_, i) => i !== index) }));

  const saveConfig = async (event) => {
    event.preventDefault();
    setSaving(true); setError('');
    try {
      const response = await axios.put(`/api/rooms/${roomCode}/interview`, { ...form, durationMinutes: Number(form.durationMinutes), candidateId: form.candidateId || null });
  ...
  const submit = async () => {
    setError(''); setSubmission(null); setSaving(true);
    try {
      const response = await axios.post(`/api/rooms/${roomCode}/interview/submit`, { sourceCode: code, language });
  ...
  const statusLabel = useMemo(() => interview?.status ? interview.status[0].toUpperCase() + interview.status.slice(1) : 'Not configured', [interview?.status]);
  const candidateName = useMemo(() => {
    if (!interview?.candidateId) return 'Any editor';
    const member = room?.members?.find((candidate) => String(candidate._id) === String(interview.candidateId));
    return member?.name || 'Assigned candidate';
  }, [interview?.candidateId, room?.members]);

  if (loading) return <section className="sidebar-section interview-section"><h3>Interview Mode</h3><p className="interview-muted">Loading…</p></section>;

  if (!interview && !isOwner) return null;

  return <section className="sidebar-section interview-section" aria-label="Interview Mode">
    <div className="section-title-row"><h3>Interview Mode</h3><span className={`interview-status status-${interview?.status || 'none'}`}>{statusLabel}</span></div>
    {error && <div className="interview-error" role="alert">{error}</div>}

    {isOwner && !interview && <div className="interview-owner-empty"><p>Create a timed coding interview with public and hidden tests.</p><button type="button" className="btn-interview-primary" onClick={() => setShowConfig(true)}>Create interview</button></div>}

    {isOwner && interview && <div className="interview-owner-controls">
      <div className="interview-summary"><strong>{interview.title}</strong><span>{candidateName} · {interview.durationMinutes} min</span></div>
      <div className="interview-actions">
        {interview.status === 'draft' && <button type="button" className="btn-interview-primary" onClick={() => transition('start')}>Start</button>}
        {interview.status === 'running' && <button type="button" className="btn-interview-secondary" onClick={() => transition('pause')}>Pause</button>}
        {interview.status === 'paused' && <button type="button" className="btn-interview-primary" onClick={() => transition('resume')}>Resume</button>}
        {['running', 'paused'].includes(interview.status) && <button type="button" className="btn-interview-danger" onClick={() => transition('end')}>End</button>}
        {interview.status !== 'running' && interview.status !== 'paused' && <button type="button" className="btn-interview-secondary" onClick={() => setShowConfig((value) => !value)}>Configure</button>}
      </div>
      {showConfig && <form className="interview-form" onSubmit={saveConfig}>
        ...
        <div className="interview-form-actions"><button type="submit" className="btn-interview-primary" disabled={saving}>{saving ? 'Saving…' : 'Save interview'}</button><button type="button" className="btn-interview-secondary" onClick={() => setShowConfig(false)}>Cancel</button></div>
      </form>}
    </div>}
...
const TestEditor = ({ title, tests, kind, updateTest, addTest, removeTest }) => <div className="test-editor"><div className="test-editor-heading"><h4>{title}</h4><button type="button" onClick={() => addTest(kind)}>+ Add</button></div>{tests.map((test, index) => <div className="test-editor-row" key={index}><input placeholder="Test name" value={test.name} onChange={(event) => updateTest(kind, index, 'name', event.target.value)} /><textarea placeholder="stdin" rows={2} value={test.stdin} onChange={(event) => updateTest(kind, index, 'stdin', event.target.value)} /><textarea placeholder="expected output" rows={2} value={test.expectedOutput} onChange={(event) => updateTest(kind, index, 'expectedOutput', event.target.value)} />{tests.length > 1 && <button type="button" className="test-remove" onClick={() => removeTest(kind, index)} aria-label={`Remove ${title} test ${index + 1}`}>Remove</button>}</div>)}</div>;
