...
export default function RoomCollaborative() {
  ...
  const status = connected ? 'Connected' : collaboration.reconnecting ? 'Reconnecting…' : 'Disconnected';
  const owner = idOf(room?.owner) === idOf(user);
  ...
  return <div className="room-page">
    <header className="room-header">
      ...
      <div className="header-actions">
        {owner && <button type="button" onClick={deleteRoom} className="btn-delete-room">Delete Room</button>}
        <div className="connection-status"><span className={`status-dot ${connected ? 'connected' : collaboration.reconnecting ? 'reconnecting' : 'disconnected'}`}></span><span>{status}</span>{crdt.crdtReady && connected && <span className="crdt-status"> · Live CRDT</span>}</div>
      </div>
    </header>
    ...
    <div className="room-layout">
      <div className="editor-section">
        <div className="editor-toolbar">
          <div className="language-selector"><label htmlFor="language">Language:</label><LanguageSelect value={language} onChange={(event) => setLanguage(event.target.value)} aria-label="Select Editor Language" /></div>
          <span className="saving-indicator" aria-live="polite">{saving ? 'Syncing…' : crdt.crdtReady ? 'CRDT Sync' : 'Initializing…'}</span>
          <button onClick={execution.handleRunCode} disabled={execution.executing || !crdt.crdtReady} className="btn-run">{execution.executing ? 'Running...' : 'Run Code'}</button>
        </div>
        <Editor
          ...
          onChange={async (value) => {
            if (value === undefined || !crdt.crdtReady) return;
            setCode(value); setSaving(true);
            try { await crdt.handleLocalChange(value); } finally { if (mounted.current) setSaving(false); }
          }}
          options={{ minimap: { enabled: true }, fontSize: 14, automaticLayout: true, scrollBeyondLastLine: false, readOnly: !crdt.crdtReady }}
        />
      </div>
      ...
      <aside className="room-sidebar" aria-label="Room Sidebar">
        <div className="sidebar-section presence-section" aria-live="polite">
          ...
          <ul className="users-list">
            ...
            {collaboration.onlineUsers.length === 0 && <li className="no-users">No other users online</li>}
          </ul>
        </div>
        ...
        <ChatPanel messages={chat.messages} messagesError={chat.messagesError} newMessage={chat.newMessage} setNewMessage={chat.setNewMessage} sendingMessage={chat.sendingMessage} connected={connected} handleSendMessage={chat.handleSendMessage} messagesEndRef={chat.messagesEndRef} />
      </aside>
    </div>
  </div>;
}
