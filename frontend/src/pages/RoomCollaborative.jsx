  ...
  return <div className={`room-page${focusMode ? ' focus-mode' : ''}`}>
    <header className="room-header">
      <div className="header-left">
        ...
        <h1>{room?.name}</h1>
        <span className={`role-badge role-${currentRole}`} aria-label={`Current role: ${currentRole}`}>{currentRole}</span>
    ...
    <div className="room-layout">
      <div className="editor-section">
        <div className="editor-toolbar">
          ...
          <button onClick={execution.handleRunCode} disabled={execution.executing || !crdt.crdtReady || !canEdit} className="btn-run" title={canEdit ? 'Run code (Ctrl/Cmd + Enter)' : 'View-only access'}>{execution.executing ? 'Running...' : canEdit ? 'Run Code' : 'View Only'}</button>
        </div>
        <Editor
          ...
          options={{ minimap: { enabled: true }, fontSize: 14, automaticLayout: true, scrollBeyondLastLine: false, readOnly: !crdt.crdtReady || !canEdit, padding: { top: 8 }, lineNumbersMinChars: 3 }}
        />
      </div>
