...
class SocketService {
  ...
  waitForConnection(timeoutMs = CONNECT_TIMEOUT_MS) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const offDisconnect = this.on('disconnect', ({ reason } = {}) => {
        if (reason === 'io server disconnect' || reason === 'io client disconnect') finish(reject, new Error(reason || 'Disconnected from collaboration server.'));
      });
    });
  }
  ...
  async joinRoom(roomCode) {
    const response = await this.emitWithAck('join-room', { roomCode }, { requirement: this.requireConnection });
